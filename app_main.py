...
elif "Orden de Produccion" in opcion_seleccionada:
                    ...
                    for emp in empleados_ordenados:
                        ...
                        if not df_asist.empty:      # Buscar si este empleado específico ya checó hoy
                            ...
                            if not emp_records.empty:
                                # 🌟 AHORA USAMOS LAS 4 COLUMNAS DIRECTAS DE LA PRIMERA FILA QUE ENCUENTRE
        ...
        with tab_checar:
            ...
            try:
                ...
                if res_hoy.status_code == 200:
                    ...
                    if registros_hoy:
                        # 🌟 Magia nueva: Como ahora es UNA sola fila por día, tomamos el primer registro
                        reg = registros_hoy[0]
                        
                        # Función rápida para limpiar nulos
                        def limpia_hora(h_raw):
                            h_str = str(h_raw).strip()
                            if h_str in ["", "None", "null"]: return "--:--"
                            return h_str[:5]

                        # Extraemos las 4 columnas directas de la base de datos
                        ...
                        if not df_asist.empty:
                            ...
                            if opcion_formato == "💰 Resumen de Nómina (Totales)":
                                if not df_asist_filt.empty:
                                    ...
                                    resumen_data = []
                                    # Agrupamos por empleado para sacar sus totales
                                    for id_emp, group in df_detalle.groupby('id_empleado'):
                                        nombre = group['nombre_empleado'].iloc[0]
                                        depto = group['depto'].iloc[0]
                                        
                                        dias_laborados = group['fecha'].nunique()
                                        ...
                                        # Lógica de penalización (3 retardos = 1 falta administrativa)
                                        penalizacion = retardos // 3 
                                        
                                        # Alerta si hay entradas sin salidas
                                    ...
                                    # Pinta los colores de los semáforos para resaltar incidencias
                                    ...
                                    st.dataframe(df_resumen_out.style.map(resaltar_alertas), width='stretch', hide_index=True)
                                else:
                            ...
                            # ==============================================================
                                ...
                                if not df_asist_filt.empty:
                                    ...
                                    records = []
                                    # 🌟 Adiós a la "magia negra" de agrupar. Ahora leemos la fila directa con sus 4 columnas
                                    for _, row in df_asist_filt.iterrows():
                                        ...
                                        records.append({
                                            ...
                                            ("👤 Datos del Empleado", "ID"): str(row['id_empleado']).strip(),
