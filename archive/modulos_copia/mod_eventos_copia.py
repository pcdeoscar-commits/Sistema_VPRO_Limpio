...
# 📡 Apuntamos al API del Laboratorio
API_URL = "https://172.16.0.20:8000"

def parsed_array(val):      
    """Auxiliar para limpiar listas que vienen de PostgreSQL como texto '{a,b,c}'"""
    if isinstance(val, list): return val
    if not val: return []
    return [x.strip().strip('"').strip("'") for x in str(val).replace("{","").replace("}","").split(",") if x]
    ...
    with tab_activas:
        ...
        if estatus_db == 'CERRADA (HISTÓRICO)':
            ...
            if d.get('id_evento') and (es_admin_coordinador or es_productor_asignado):  # 🔒 Solo el Admin, Coordinador o el Productor de la OP pueden ver esto
                ...
                activar_gira = st.toggle("💼 SI EL EVENTO ES FUERA DE LA CIUDAD ... ACTIVE ESTA BITÁCORA", key=f"tgl_gira_{id_actual}") # El Switch Principal
                    ...
                    if not pers_sel:
                        ...
                        with st.container(border=True):
                            ...
                            try:
                                ...
                                st.error("📡 Fallo de conexión al consultar el historial de asistencia.")
                            ...
                            for fecha_str in fechas_registradas:
                                ...
                                with st.expander(f"🔒 Jornada Registrada: {fecha_str}", expanded=st.session_state[llave_edicion]):
                                    ...
                                    if not st.session_state[llave_edicion]: # MODO LECTURA: Mostrar la tabla bloqueada y el botón de abrir
                                        ...
                                        st.warning("✏️ Estás editando esta jornada. Puedes borrar las horas de los que no viajaron o modificar las comidas.")
                                        ...
                                        if c_btn1.button("💾 Guardar Correcciones", type="primary", use_container_width=True, key=f"btn_guardar_{fecha_str}_{id_actual}"):
                                            ...
                                            for idx, row in df_editado.iterrows():
                                                registros_modificados.append({
                                                    ...
                                                    "num_empleado": 0, 
                                                    "nombre_empleado": str(row['Empleado']),
                                                    "depto": "STAFF VPRO",
