    ...
    try:
        ...
        if row:
            return {
                ...
                "completo": row["hora_salida_v"] is not None  # 👈 Sabe que el día termina hasta la salida vespertina
            }
...
def registrar_tarjetazo_asistencia(payload: ChecadaPayload):
    ...
    try:
        with engine_personal.begin() as conn:
            # 🔍 Buscamos la fila de HOY de este empleado
            ...
            # ESCENARIO 1: Es su primer tarjetazo del día (Mañana)
            if not registro_hoy:
                conn.execute(text("""
            ...
            # ESCENARIO 2: Ya entró en la mañana, pero no ha salido (A comer)
            if registro_hoy["hora_entrada"] is not None and registro_hoy["hora_salida"] is None:
                conn.execute(text("""
                    ...
                    SET hora_salida = CAST(:hora AS time), 
                        observaciones = COALESCE(observaciones, '') || ' | Salida M: ' || :obs
                    WHERE id_registro = :id
                """), {"hora": hora_str, "obs": payload.observaciones, "id": id_reg})
                return {"status": "SUCCESS", "mensaje": "✅ SALIDA MATUTINA REGISTRADA"}

            # ESCENARIO 3: Ya salió a comer, regresa en la tarde
            elif registro_hoy["hora_salida"] is not None and registro_hoy["hora_entrada_v"] is None:
                conn.execute(text("""
                    UPDATE public.control_asistencia 
                    SET hora_entrada_v = CAST(:hora AS time), 
                        observaciones = COALESCE(observaciones, '') || ' | Entrada V: ' || :obs
                    WHERE id_registro = :id
                """), {"hora": hora_str, "obs": payload.observaciones, "id": id_reg})
                return {"status": "SUCCESS", "mensaje": "✅ ENTRADA VESPERTINA REGISTRADA"}

            # ESCENARIO 4: Ya entró en la tarde, se va a su casa
            elif registro_hoy["hora_entrada_v"] is not None and registro_hoy["hora_salida_v"] is None:
                conn.execute(text("""
                    UPDATE public.control_asistencia 
                    SET hora_salida_v = CAST(:hora AS time), 
                        estatus = 'COMPLETO',
                        observaciones = COALESCE(observaciones, '') || ' | Salida V: ' || :obs
                    WHERE id_registro = :id
                """), {"hora": hora_str, "obs": payload.observaciones, "id": id_reg})
                return {"status": "SUCCESS", "mensaje": "✅ SALIDA VESPERTINA REGISTRADA"}

            # ESCENARIO 5: Un loquito pasó el gafete 5 veces el mismo día
            else:
                raise HTTPException(status_code=400, detail="Tranquilo, ya completaste tus 4 registros de hoy.")

    except HTTPException as he: 
        raise he
    except Exception as e: 
        print(f"🔥 ERROR CRÍTICO EN CHECADOR: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def obtener_reporte_asistencia_global():
    ...
    query = text("""
        SELECT c.id_registro, c.fecha, TRIM(c.id_empleado), encode(e.nombre::bytea, 'hex') as nombre_empleado, 
               ...
               c.hora_entrada_v, c.hora_salida_v  -- 👈 AQUÍ AGREGAMOS LAS COLUMNAS DE LA TARDE
        FROM public.control_asistencia c
    ...
    try:
        ...
        return [{
            ...
            "estatus": r[7],
            "observaciones": r[8],
            "hora_entrada_v": str(r[9]) if r[9] else None,   # 👈 LO EMPAQUETAMOS EN EL JSON
            "hora_salida_v": str(r[10]) if r[10] else None   # 👈 LO EMPAQUETAMOS EN EL JSON
        } for r in rows]
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
