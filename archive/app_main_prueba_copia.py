...
if opcion_seleccionada == "🏠 Inicio":
    ...
    st.markdown("<h4 style='color:#0f172a; margin-top: 25px;'>⏱️ Mi Asistencia de Hoy</h4>", unsafe_allow_html=True)     # --- MUDANZA: BITÁCORA DE ASISTENCIA EN EL INICIO ---
    try:
        import pandas as pd
        import datetime
        ...
        # Consultamos ambas bases de datos usando el API_URL de tu main
        ...
        # Lógica de fusión (REGLA DE ORO: CHECADOR FÍSICO MANDA AL 100%)
        if registros_oficina or registros_locacion:
            def limpia_hora(h_raw):
                h_str = str(h_raw).strip()
                # 👻 Si la base de datos trae 00:00:00, lo mostramos como vacío
