...
# Importacion segura de ReportLab
try:
...
def generar_pdf_gastos(resumen, df_detalle):
    ...
    story.append(Paragraph("INFORME DE RENDICION DE CUENTAS Y COMPROBACION DE GASTOS", ParagraphStyle('Sub', parent=title_style, fontSize=11, leading=14)))
    story.append(Spacer(1, 15))
    
    data_encabezado = [
        [Paragraph(f"<b>Folio Evento:</b> {resumen.get('folio', '')}", label_style), Paragraph(f"<b>Empleado:</b> {resumen.get('nombre', '')}", label_style)],
        [Paragraph(f"<b>Departamento:</b> {resumen.get('depto', '')}", label_style), Paragraph(f"<b>Vehiculo/Unidades:</b> {resumen.get('vehiculo', '')}", label_style)],
        ...
        [Paragraph(f"<b>Remanente/Devolucion:</b> ${resumen.get('restante', 0.0):,.2f}", label_style), Paragraph(f"<b>Rango Odometro:</b> {resumen.get('km_i', 0):,} - {resumen.get('km_f', 0):,} KM", label_style)]
    ]
    ...
    sig_data = [
        ...
        ["Firma del Productor Responsable", "", "Firma de Auditoria y Administracion"],
        [str(resumen.get('nombre', 'Produccion')), "", "Direccion VPRO"]
    ]
    
    t_sig = Table(sig_data, colWidths=[220, 100, 220])
...
def renderizar_modulo(API_URL):
    operador = str(st.session_state.get('usuario_actual') or 'OPERADOR').upper()
    id_usuario = str(st.session_state.get('id_usuario') or '000')
    depto_usuario = str(st.session_state.get('depto') or 'OFICINA')
    rol_usuario = str(st.session_state.get('role') or st.session_state.get('rol') or '').upper()
    
    es_auditor = rol_usuario in ["ADMIN", "COORDINADOR"] or "ANA" in operador or "LILIA" in operador
    es_ana_lilia_directiva = "ANA" in operador or "LILIA" in operador or rol_usuario in ["ADMIN", "COORDINADOR"]
    
    st.markdown("## 💸 Rendición de Cuentas y Control de Gastos Operativos")

    with st.expander("❓ Ayuda Contextual: ¿Cómo funciona la Comprobación y Auditoría Financiera?"):
        st.markdown("""
        ### 🧾 FASE 5: Rendición de Cuentas (Módulo del Productor)
        * 🔓 **Acceso al Reporte:** El sistema muestra solo las OPs dirigidas por ti y que **ya fueron liberadas por Bodega**.
        * 🚗 **Odómetros:** Se exige capturar el Kilometraje Inicial y Final de los vehículos.
        * 📊 **Matriz Dinámica de Gastos:** Ingresa el gasto diario en Hotel, Alimentos, Casetas, etc.
        * 🧮 **Cálculo Automático:** Ingresa el "Presupuesto Entregado", el sistema resta lo gastado y dicta el Remanente a devolver.
        * 🖋️ **Firma Electrónica:** Al enviar el informe, la OP se bloquea para el Productor y se manda directo a Administración.

        ---
        ### 🔎 FASE 6: Auditoría y Cierre Financiero (Módulo de Dirección)
        * 📥 **Buzón de Entrada:** Administración recibe alertas con los informes recién enviados.
        * 🧾 **Cuadre Físico:** Se cotejan tickets, facturas y efectivo sobrante contra lo que dicta el sistema.
        * ✅ **Sello Definitivo:** Si no hay discrepancias, se presiona **"✅ MARCAR COMO REVISADO Y APROBADO"**.
        * 📁 **Bóveda Histórica:** La OP **desaparece de todas las listas de Pendientes** y se transfiere al "Historial de Informes Archivados". El evento queda oficialmente cerrado y blindado.
        
        🚨 **Corrección de Errores:** Si el Productor se equivocó al capturar un gasto, Administración tiene el privilegio exclusivo de editar y corregir la celda directamente desde el panel de Auditoría.
        """)
    ...
    with tab_captura:
        ...
        try:
            ...
            if res_fol.status_code == 200:
                ...
                st.caption(f"⚠️ Nota de Caja Chica: No se pudieron indexar folios pendientes.")
        except Exception: 
            st.caption("💡 Módulo financiero: Sincronizando marcas de red...")
        ...
        if "---" in sel_folio: 
            ...
            try:
                ...
                print(f"⚠️ SILENCED ERROR in mod_reporte_gastos.py: {e}")
        
            ev_clean = {}
            if isinstance(ev_data, dict):
                ev_clean = {str(k).lower().replace('_', '').replace('-', '').strip(): v for k, v in ev_data.items()}
            ...
            if fec_inst:
                ...
                except Exception as e: print(f"⚠️ SILENCED ERROR in mod_reporte_gastos.py: {e}")
            ...
            if raw_personal:
                ...
                except Exception: p_num = 0
        
            responsable_evento = resp_de_produccion
            responsable_clean = str(responsable_evento).upper().strip()
            usuario_actual_clean = str(st.session_state.get('usuario_actual') or '').upper().strip()

            if (usuario_actual_clean in responsable_clean) or (responsable_clean in ["", "NONE", "NO ASIGNADO", "NULL"]) or (rol_usuario in ['ADMIN', 'COORDINADOR']):
                ...
                with st.container(border=True):
                    ...
                    c_top1.text_input("👤 Empleado que rinde:", value=operador, disabled=True, key=f"rep_u_{folio_id}")
                ...
                if st.button("💾 ENVIAR INFORME COMPLETO Y ARCHIVAR EVENTO", type="primary", use_container_width=True, key="btn_save_gastos_final"):
                    ...
                    if res_save.status_code == 200:
                        ...
                        # Limpiar el selectbox para que la OP ya no aparezca seleccionada al recargar
                        if "gastos_master_folio_sel" in st.session_state:
                            del st.session_state["gastos_master_folio_sel"]
                        time.sleep(1.5)
                        st.rerun()
                    else: 
                        st.error(f"❌ Error al guardar informe: {res_save.text}")
            else:
                st.info(f"ℹ️ **Responsable en OP:** {responsable_evento} | **Firmado en Terminal:** {operador}")
    ...
    if es_auditor:
        with tab_auditoria:
            ...
            dias_es_label = ["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"]
            ...
            informes_pendientes = [x for x in lista_informes if not x.get("revisado")]
            informes_archivados = [x for x in lista_informes if x.get("revisado")]
            ...
            with sub_tab_buzon:
                if not informes_pendientes:
                    st.success("No hay informes pendientes por auditar.")
                else:
                    mapa_pendientes = {}
                    opciones_pendientes = []
                    ...
                    try:
                        ...
                        if res_comp.status_code == 200:
                            ...
                            # ✨ RECONSTRUCCIÓN DE FECHAS PARA PDF Y TABLA DE AUDITORÍA
                            f_base_p = pd.to_datetime(mae['periodo_desde']).date()
                            rows_tabla_audit = []
                            for d in det:
                                f_obj = f_base_p + datetime.timedelta(days=d['dia_num'] - 1)
                                f_str = f"{f_obj.strftime('%d/%m')} - {dias_es_label[f_obj.weekday()]}"
                                rows_tabla_audit.append({"Fecha": f_str, "Hotel": d['hotel'], "Transp": d['transporte'], "Combust": d['combustible'], "Casetas": d['casetas'], "Desay": d['desayuno'], "Comida": d['comida'], "Cenas": d['cenas'], "Varios": d['varios'], "Total": d['total_dia']})
                        ...
                        st.error(f"❌ Error al cargar expediente: {e}")
                ...
                if not informes_archivados:
                    ...
                    try:
                        ...
                        if res_comp_h.status_code == 200:
                            ...
                            # ✨ RECONSTRUCCIÓN DE FECHAS PARA HISTORIAL ANA LILIA
                            f_base_h = pd.to_datetime(mae_h['periodo_desde']).date()
                            rows_tabla_h = []
                            for d in det_h:
                                f_obj = f_base_h + datetime.timedelta(days=d['dia_num'] - 1)
                                f_str = f"{f_obj.strftime('%d/%m')} - {dias_es_label[f_obj.weekday()]}"
                                rows_tabla_h.append({"Fecha": f_str, "Hotel": d['hotel'], "Transp": d['transporte'], "Combust": d['combustible'], "Casetas": d['casetas'], "Desay": d['desayuno'], "Comida": d['comida'], "Cenas": d['cenas'], "Varios": d['varios'], "Total": d['total_dia']})
                                ...
                                if st.button("💾 GUARDAR CORRECCIONES AL HISTÓRICO", type="primary", use_container_width=True):
                                    ...
                                    for idx_mod, row_mod in df_h_editado.iterrows():
                                        ...
                                        payload_mod["detalles"].append({
                                            "dia_num": int(idx_mod) + 1, # ✨ FIX: Salva de regreso el dia numerico 1, 2, 3...
                        ...
                        st.error(f"❌ Error al jalar registro histórico: {e}")
        ...
        with tab_mi_historial:
            ...
            try:
                ...
                if res_aud.status_code == 200: 
                    lista_mis_informes = [x for x in res_aud.json() if x.get("revisado") and str(x.get("nombre_empleado")).strip().upper() == operador]
            except Exception as e: print(f"⚠️ SILENCED ERROR in mod_reporte_gastos.py: {e}")
            ...
            if not lista_mis_informes:
                ...
                try:
                    ...
                    if res_comp_m.status_code == 200:
                        ...
                        # ✨ RECONSTRUCCIÓN DE FECHAS PARA HISTORIAL PRODUCTORES
                        f_base_m = pd.to_datetime(mae_m['periodo_desde']).date()
                        rows_tabla_m = []
                        cats_grid_master = ["Hotel", "Transp", "Combust", "Casetas", "Desay", "Comida", "Cenas", "Varios"]
                        for d in det_m:
                            f_obj = f_base_m + datetime.timedelta(days=d['dia_num'] - 1)
                            f_str = f"{f_obj.strftime('%d/%m')} - {dias_es_label[f_obj.weekday()]}"
                            rows_tabla_m.append({"Fecha": f_str, "Hotel": d['hotel'], "Transp": d['transporte'], "Combust": d['combustible'], "Casetas": d['casetas'], "Desay": d['desayuno'], "Comida": d['comida'], "Cenas": d['cenas'], "Varios": d['varios'], "Total": d['total_dia']})
                    ...
                    st.error(f"❌ Error al cargar tu historial: {e}")